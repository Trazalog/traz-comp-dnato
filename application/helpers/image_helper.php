<?php defined('BASEPATH') or exit('No direct script access allowed');

if (!function_exists('form')) {


    function imageAdmin($img64,$imgName){

        $image = '';

        $placeholder = "data:image/png;base64,";

        if (isset($img64, $imgName) && $imgName !== '' && $imgName !== null && $img64 !== null && $img64 !== '') {

            if (!function_exists('obtenerExtension')) {
                require_once(APPPATH . 'modules/traz-comp-formularios/helpers/form_helper.php');
            }
            $ext = obtenerExtension($imgName);
            if ($ext === '') {
                $ext = 'data:image/jpeg;base64,';
            }

            $blob = $img64;
            if (is_string($img64) && function_exists('pg_unescape_bytea')) {
                $blob = pg_unescape_bytea($img64);
            }
            if ($blob === false || $blob === '') {
                $image = $placeholder;
            } else {
                $normalized = preg_replace('/\s+/', '', $blob);
                $alreadyB64 = strlen($normalized) > 32
                    && preg_match('/^[A-Za-z0-9+\/]+=*$/', $normalized)
                    && base64_decode($normalized, true) !== false;
                $payload = $alreadyB64 ? $normalized : base64_encode($blob);
                $image = $ext . $payload;
            }
            //$style = "background-image: url($image);";
        }else{
            $image = $placeholder;
        }
    
        if($image == 'data:image/png;base64,0' || $image == 'data:image/jpg;base64,0' || $image == 'data:image/jpeg;base64,0'){
            $image = $placeholder;
        }
        
        /*log_message('debug','#TRAZA | IMAGEN SESION | imageAdmin()  >> '.$imgName. ' :: '.$img64.' :: '.$image);*/
    
        return $image;
    }

    // NOTA: obtenerExtension() se define en
    // application/modules/traz-comp-formularios/helpers/form_helper.php (sin function_exists wrap).
    // Se carga on-demand dentro de imageAdmin() para evitar el conflicto de redeclaración.
}


?>