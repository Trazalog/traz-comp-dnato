<?php
/**
 * Login OAuth (MCP) — Paso 2: selección de empresa.
 *
 * Se muestra sólo cuando el usuario ya validó sus credenciales y Bonita le
 * reporta más de una empresa. Cada empresa es un botón: una tarjeta con su
 * logo y su nombre. Si la empresa no tiene logo cargado, imageAdmin()
 * devuelve un placeholder genérico, así que la tarjeta nunca queda vacía.
 *
 * Es la contraparte de views/login_empresa.php (el login web): mismo CSS,
 * mismo fondo y mismas imágenes, para que las dos entradas al sistema se
 * lean igual. La diferencia es que esta vista es standalone —abre y cierra
 * su propio HTML— porque el flujo OAuth no pasa por views/header.php.
 *
 * Variables esperadas:
 *   $empresas    array de objetos con empr_id, descripcion, nombre, image, imagepath
 *   $csrf_token  token generado en el Paso 1 (oauth_csrf)
 *   $logoEmpresa logo del sitio (core.tablas / configuraciones_ui)
 *   $error       mensaje a mostrar, o '' si no hay
 */
defined('BASEPATH') OR exit('No direct script access allowed');

$mensajeTexto = isset($error) ? (string) $error : '';
$imagenFondo  = defined('LOGIN_IMG_BACKGROUND') ? LOGIN_IMG_BACKGROUND : 'public/img/toolslogin.jpg';

// core.tablas guarda la ruta relativa; si alguien cargara una URL absoluta,
// base_url() la concatenaria y rompería el src.
$logoSrc = '';
if (!empty($logoEmpresa)) {
    $logoSrc = preg_match('~^(https?:)?//~i', $logoEmpresa)
        ? $logoEmpresa
        : base_url($logoEmpresa);
}
?>
<!doctype html>
<html lang="es">
<head>
    ...
    <title>Trazalog — Seleccioná tu empresa</title>
</head>
<body>
<style>
    html, body {
...
<div class="tz-sel">
    <div class="tz-sel__card">
        ...
        <div class="tz-sel__head">
            <?php if ($logoSrc !== ''): ?>
                <img src="<?php echo html_escape($logoSrc); ?>" alt="Trazalog Tools" class="tz-sel__logo">
            <?php endif; ?>
            <h1 class="tz-sel__title">¿Con qué empresa querés ingresar?</h1>
            <p class="tz-sel__subtitle">
                Tu usuario pertenece a más de una. Hacé clic en una para autorizar el acceso.
            </p>
        </div>

        <?php if ($mensajeTexto !== ''): ?>
            <div class="tz-sel__alert"><?php echo html_escape($mensajeTexto); ?></div>
        <?php endif; ?>
                ...
                <div class="tz-sel__item">
                    <?php
                      /* Un formulario por tarjeta, con el empr_id en un hidden.
                         ...
                         lugar de su value. */
                      echo form_open(base_url() . 'oauth/login/empresa');
                    ?>
                        <input type="hidden" name="oauth_csrf" value="<?php echo html_escape($csrf_token); ?>">
        ...
        <div class="tz-sel__foot">
            <a href="<?php echo base_url(); ?>oauth/login">Ingresar con otro usuario</a>
        </div>
